      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < ptsx.size(); i++) {
            double x = ptsx[i] - px;
            double y = ptsy[i] - py;
            waypointsX[i] = x * cos(-psi) - y * sin(-psi);
            waypointsY[i] = x * sin(-psi) + y * cos(-psi);
          }
          ...
          */
          ...
          double predX = 0.0 + v * dt;
          const double predY = 0.0; 
          double predPsi = 0.0 + v * -delta / Lf * dt;
          double predV = v + a * dt;
          double predCte = cte + v * sin(epsi) * dt;
          double predEpsi = epsi + v * -delta / Lf * dt;
          
          // Feed in the predicted state values
          Eigen::VectorXd state(6);
          state << predX, predY, predPsi, predV, predCte, predEpsi;
